...
- **Run `pip-audit`** (or equivalent) before every release
...
`pip-audit` catches known CVEs; it won't catch a malicious or typosquatted package. Also:

- **Commit the lockfile** and install with `uv sync --frozen` (not `uv sync`) in CI — reproducible builds, no silent version drift.
...
### Supply Chain
- [ ] Lockfile committed; CI installs with `uv sync --frozen`
