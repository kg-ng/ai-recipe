...
pip-audit --audit-level=critical

# Keep dependencies updated
npx npm-check-updates
```

**Supply-chain hygiene** (`pip-audit` won't catch malicious packages):
- [ ] Lockfile committed; CI installs with `uv sync --frozen` (not `uv sync`)
...
| 6 | Vulnerable Components | `pip-audit`, keep deps updated, minimal deps |
